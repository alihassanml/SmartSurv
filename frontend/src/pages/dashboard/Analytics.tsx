...
const Analytics: React.FC = () => {
  ...
  const sevStyle: Record<string, { color: string; bg: string; border: string }> = {
    HIGH:   { color: '#ffb4ab', bg: 'rgba(147,0,10,0.2)',   border: 'rgba(255,180,171,0.5)' },
    MEDIUM: { color: '#ffb4ab', bg: 'rgba(255,180,171,0.1)', border: 'rgba(255,180,171,0.3)' },
    LOW:    { color: '#b0c6ff', bg: 'rgba(176,198,255,0.06)', border: 'rgba(176,198,255,0.2)' },
    MATCH:  { color: '#ffb4ab', bg: 'rgba(255,180,171,0.12)', border: 'rgba(255,180,171,0.4)' },
  };

  return (
    <div className="h-full flex flex-col overflow-hidden" style={{ background: '#0c0e11' }}>
        ...
        {viewingAlert && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
            ...
            <motion.div
              ...
              style={{ background: '#111316', border: '1px solid rgba(176,198,255,0.15)', borderRadius: '0.5rem' }}
            >
              {/* Corner accents */}
              {['top-0 left-0 border-t-2 border-l-2', 'top-0 right-0 border-t-2 border-r-2',
                'bottom-0 left-0 border-b-2 border-l-2', 'bottom-0 right-0 border-b-2 border-r-2'].map((cls, i) => (
                <div key={i} className={`absolute w-6 h-6 ${cls} z-10`} style={{ borderColor: 'rgba(176,198,255,0.3)' }} />
              ))}

              {/* Image */}
              <div className="flex-1 bg-black/40 relative flex items-center justify-center overflow-hidden min-h-[260px]">
                {/* Scan line */}
              ...
              {/* Info panel */}
              <div className="w-full md:w-72 flex flex-col p-6 gap-5" style={{ background: '#1a1c1f', borderLeft: '1px solid rgba(176,198,255,0.08)' }}>
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-[9px] font-bold tracking-[0.3em] uppercase mb-1" style={{ color: 'rgba(176,198,255,0.4)' }}>Incident Data</p>
                    <p className="text-base font-bold uppercase" style={{ color: '#ccd8ff', fontFamily: "'Manrope', sans-serif" }}>
                      {getSeverity(viewingAlert)} ALERT
                    </p>
                  </div>
                  <button onClick={() => setViewingAlert(null)} style={{ color: '#8c909f' }}>
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div className="space-y-4 flex-1">
                  <div>
                    <p className="text-[9px] uppercase tracking-widest mb-2" style={{ color: 'rgba(176,198,255,0.35)' }}>Timestamp</p>
                    <p className="text-xs font-bold" style={{ color: '#b0c6ff' }}>{viewingAlert.timestamp}</p>
                  </div>
                  <div>
                    <p className="text-[9px] uppercase tracking-widest mb-2" style={{ color: 'rgba(176,198,255,0.35)' }}>Detections</p>
                  ...
                  {viewingAlert.is_person_search_match && (
                    <div className="flex items-center gap-2 px-3 py-2"
                      ...
                      <span className="text-[10px] font-bold tracking-widest" style={{ color: '#ffb4ab' }}>WATCHLIST MATCH</span>
                    </div>
                  )}
                </div>
      ...
      <div className="shrink-0 flex items-center justify-between px-6 py-3"
        ...
        <div className="flex gap-1 p-1" style={{ background: 'rgba(176,198,255,0.04)', borderRadius: '0.25rem', border: '1px solid rgba(176,198,255,0.08)' }}>
          {(['charts', 'live'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className="px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest transition-all"
              style={tab === t
                ? { background: '#0a58ca', color: '#ccd8ff', borderRadius: '0.125rem' }
        ...
        {tab === 'live' && (
          <button onClick={handleExportCSV}
            ...
            onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(176,198,255,0.08)'; }}
        ...
        {tab === 'charts' ? (
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: 'Total Alerts', value: metrics.total, icon: Activity, color: '#b0c6ff' },
                { label: 'Critical', value: metrics.high, icon: AlertTriangle, color: '#ffb4ab' },
                { label: 'WL Matches', value: metrics.matches, icon: User, color: '#ffb4ab' },
                { label: 'Today', value: metrics.today, icon: Shield, color: '#b0c6ff' },
              ].map((kpi, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
                  className="p-4" style={{ background: '#1a1c1f', border: '1px solid rgba(176,198,255,0.08)', borderRadius: '0.375rem' }}>
                  <div className="flex justify-between items-start mb-3">
                    <p className="text-[9px] font-bold uppercase tracking-widest" style={{ color: 'rgba(176,198,255,0.4)' }}>{kpi.label}</p>
            ...
            <div className="flex items-center gap-3">
              <span className="text-[9px] font-bold uppercase tracking-widest" style={{ color: 'rgba(176,198,255,0.3)' }}>Filter</span>
              <div className="flex gap-1 p-1" style={{ background: 'rgba(176,198,255,0.04)', border: '1px solid rgba(176,198,255,0.08)', borderRadius: '0.25rem' }}>
            ...
            <div className="space-y-2">
              <AnimatePresence>
                {filtered.length === 0 ? (
                  <div className="py-20 flex flex-col items-center gap-3" style={{ color: 'rgba(176,198,255,0.2)' }}>
                  ...
                  return (
                    <motion.div
                      ...
                      style={{ background: '#1a1c1f', border: `1px solid rgba(176,198,255,0.08)`, borderRadius: '0.25rem', cursor: 'default' }}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          {alert.is_person_search_match && (
                            <span className="text-[9px] font-bold flex items-center gap-1" style={{ color: '#ffb4ab' }}>
                              <User className="w-2.5 h-2.5" /> WL_MATCH
                            </span>
                          )}
                          <span className="text-[9px] ml-auto" style={{ color: 'rgba(176,198,255,0.3)' }}>{alert.timestamp}</span>
                        </div>
                      ...
                      <button
                        ...
                        style={{ border: '1px solid rgba(176,198,255,0.2)', color: '#b0c6ff', borderRadius: '0.25rem' }}
                        onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = '#0a58ca'; (e.currentTarget as HTMLButtonElement).style.borderColor = '#0a58ca'; }}
                        onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = 'transparent'; (e.currentTarget as HTMLButtonElement).style.borderColor = 'rgba(176,198,255,0.2)'; }}
                      >
