...
const Analytics: React.FC = () => {
  ...
  const sevStyle: Record<string, { color: string; bg: string; border: string }> = {
    ...
    LOW:    { color: '#2480ff', bg: 'rgba(36,128,255,0.06)', border: 'rgba(36,128,255,0.15)' },
    MATCH:  { color: '#ba1a1a', bg: 'rgba(186,26,26,0.07)',  border: 'rgba(186,26,26,0.25)' },
  };

  return (
    <div className="h-full flex flex-col overflow-hidden" style={{ background: '#e8ecf0' }}>
        ...
        {viewingAlert && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
            ...
            >
              ...
              {/* Info panel */}
              <div className="w-full md:w-72 flex flex-col p-6 gap-5" style={{ background: '#f2f4f6', borderLeft: '1px solid rgba(0,0,0,0.1)' }}>
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-[9px] font-bold tracking-[0.3em] uppercase mb-1" style={{ color: '#74777d' }}>Incident Data</p>
                    <p className="text-base font-bold uppercase" style={{ color: '#191c1e', fontFamily: "'Manrope', sans-serif" }}>
                      {getSeverity(viewingAlert)} ALERT
                    </p>
                  </div>
                  <button onClick={() => setViewingAlert(null)} style={{ color: '#74777d' }}>
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div className="space-y-4 flex-1">
                  <div>
                    <p className="text-[9px] uppercase tracking-widest mb-2" style={{ color: '#74777d' }}>Timestamp</p>
                    <p className="text-xs font-bold" style={{ color: '#2480ff' }}>{viewingAlert.timestamp}</p>
                  </div>
                  <div>
                    <p className="text-[9px] uppercase tracking-widest mb-2" style={{ color: '#74777d' }}>Detections</p>
                ...
                {viewingAlert.image && (
                  <a href={`data:image/jpeg;base64,${viewingAlert.image}`} download={`snapshot_${Date.now()}.jpg`}
                    ...
                    style={{ background: '#2480ff', color: '#ffffff', borderRadius: '0.25rem' }}>
        ...
        <div className="flex gap-1 p-1" style={{ background: 'rgba(0,0,0,0.03)', borderRadius: '0.25rem', border: '1px solid rgba(0,0,0,0.1)' }}>
          {(['charts', 'live'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              ...
              style={tab === t
                ? { background: '#2480ff', color: '#ffffff', borderRadius: '0.125rem' }
        ...
        {tab === 'charts' ? (
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: 'Total Alerts', value: metrics.total, icon: Activity, color: '#2480ff' },
                ...
                { label: 'Today', value: metrics.today, icon: Shield, color: '#2480ff' },
              ].map((kpi, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
                  className="p-4" style={{ background: '#f2f4f6', border: '1px solid rgba(0,0,0,0.1)', borderRadius: '0.375rem' }}>
                  <div className="flex justify-between items-start mb-3">
                    <p className="text-[9px] font-bold uppercase tracking-widest" style={{ color: '#74777d' }}>{kpi.label}</p>
            ...
            <div className="flex items-center gap-3">
              <span className="text-[9px] font-bold uppercase tracking-widest" style={{ color: '#74777d' }}>Filter</span>
            ...
            <div className="space-y-2">
              <AnimatePresence>
                {filtered.length === 0 ? (
                  <div className="py-20 flex flex-col items-center gap-3" style={{ color: '#c4c6cc' }}>
                  ...
                  return (
                    <motion.div
                      ...
                      style={{ background: '#f2f4f6', border: `1px solid rgba(0,0,0,0.1)`, borderRadius: '0.25rem', cursor: 'default' }}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          <span className="text-[9px] ml-auto" style={{ color: '#74777d' }}>{alert.timestamp}</span>
                        </div>
                      ...
                      <button
                        ...
                        style={{ border: '1px solid rgba(36,128,255,0.15)', color: '#2480ff', borderRadius: '0.25rem' }}
                        onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = '#2480ff'; (e.currentTarget as HTMLButtonElement).style.borderColor = '#2480ff'; }}
